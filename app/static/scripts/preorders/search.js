async function fetchPreorders(dateValue) {
    ...
    try {
        ...
        // Очистка перед выводом новых данных
        searchInfoContainer.innerHTML = ''; 
        requestsList.innerHTML = ''

        if (result.data === null) {
            searchInfoContainer.innerHTML = `<span class="d-flex justify-content-center align-items-center" style=" color: #dc3545; padding: 20px;">Заказы на эту дату не найдены</span>`;
            return;
        } else {
        ...
        // Если бэкенд или API Деловых Линий вернули 500
...
document.getElementById('preorder_form').addEventListener('submit', function(event) {
    ...
    // onsubmit фетч данных с бэка
    fetchPreorders(dateValue);
});

// Сброс выведенных данных и обновление URL
function resetPage() {
